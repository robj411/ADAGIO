...
print(system.time(for(direct_VE in c(0,0.6)){
  for (simnum in 1:nsim) {
    ...
    trial_outcomes <- foreach(tr = 1:trials) %dopar% {
      bTrial <- bTrials[tr]
    ...
    for(tr in 1:trials){
      ...
      ss[index,simnum] <- trial_outcomes[[tr]]$analysed_trialsize
...
results_tab[,5] <- apply(mle_pvals,2,function(x)sum(x<0.05,na.rm=T)/sum(!is.na(x)))
results_tab[,6] <- apply(mle_pvals0,2,function(x)sum(x<0.05,na.rm=T)/sum(!is.na(x)))
