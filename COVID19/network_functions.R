...
spread <- function( s_nodes, v_nodes, e_nodes_info, current_infectious, direct_VE,incperiod_shape, incperiod_rate,susc_list=contact_list,beta_scalar=1){
  ...
  if(sum(v_nodes)>0){
    ...
    newinfected <- c(newinfected,funique(infectees_susc))#funique(c(infectees_susc,infectees_hr_susc,infectees_n_susc)))
  }
...
recover <- function(e_nodes_info,i_nodes_info,infperiod_shape,infperiod_rate,cluster_people_index) {
...
simulate_contact_network <- function(neighbour_scalar,high_risk_scalar,first_infected,
  ...
  e_nodes_info <- rbind(e_nodes_info,c(first_infected,rep(0,length(first_infected)),inc_time))
  s_nodes[first_infected] <- 0
  e_nodes[first_infected] <- 1
  
  #recruitment_time <- round(rgamma(1,shape=recruit_shape,rate=recruit_rate))
  recruitment_time <- ceiling(rtruncnorm(1,a=0,mean=recruit_mean,sd=recruit_sd) + inc_time)
  ...
  for(time_step in 1:sim_time){
    ...
    if ((nrow(e_nodes_info)>0)||(nrow(i_nodes_info)>0)) {
      rec_list <- recover(e_nodes_info,i_nodes_info,infperiod_shape,infperiod_rate,cluster_people_index)
