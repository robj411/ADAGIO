...
compute_grid <- function(type){
  ...
  for(iter in 1:nIter){
    ...
    netwk <- simulate_contact_network(first_infected,inf_time=inf_period,start_day=iter,from_source=0,cluster_flag=0,direct_VE=direct_VE,individual_recruitment_times = T,spread_wrapper = covid_spread_wrapper)
