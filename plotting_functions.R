...
ys <- rlnorm(1000)
pvals <- abs(ys - xs)
binx <- equal_freq_disc(xs)
x_binned <- binx[[1]]
x_breaks <- binx[[2]]
biny <- equal_freq_disc(ys)
y_binned <- biny[[1]]
y_breaks <- biny[[2]]
...
for(i in 1:x_points){
  for(j in 1:y_points){
    ...
    if(length(grid_pvals)>0){
      grid_pval[i,j] <- sum(grid_pvals<0.5)/length(grid_pvals)
    }
  }
}
...
color.legend(max(x_breaks)+0.5,0,max(x_breaks)+1,max(y_breaks),col.labels,rev(redCol),gradient="y",cex=1,align="rb")
#dev.off()
